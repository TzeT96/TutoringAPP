...
const ITEMS_PER_PAGE = 8; // Number of students per page
...
const CourseDetailPage = () => {
  ...
  const getPaginatedResults = (assignmentId: string) => {
    const filteredResults = getFilteredStudentResults(assignmentId);
    const page = currentPage[assignmentId] || 1;
  ...
  const handlePageChange = (assignmentId: string, page: number) => {
    setCurrentPage(prev => ({
      ...prev,
      [assignmentId]: page
    }));
                      ...
                      <div className="space-y-1 flex-grow">
                        <div className="flex justify-between items-center mb-4">
                          <div className="flex items-center space-x-2">
                            ...
                            >
                              <svg
                                className={`h-5 w-5 transform transition-transform ${dropdownOpen[assignment.id] ? 'rotate-180' : ''}`}
                                fill="none"
                                viewBox="0 0 24 24"
                                stroke="currentColor"
                              >
                    ...
                    <>
                      <table className="min-w-full divide-y divide-gray-200">
                        ...
                        <tbody className="bg-white divide-y divide-gray-200">
                          {getPaginatedResults(assignment.id).results.map((result) => {
                      ...
                      <div className="bg-white px-4 py-3 flex items-center justify-between border-t border-gray-200 sm:px-6">
                        <div className="flex-1 flex justify-between sm:hidden">
                          <button
                            onClick={() => handlePageChange(assignment.id, (currentPage[assignment.id] || 1) - 1)}
                            disabled={(currentPage[assignment.id] || 1) <= 1}
                          ...
                          <button
                            onClick={() => handlePageChange(assignment.id, (currentPage[assignment.id] || 1) + 1)}
                            disabled={(currentPage[assignment.id] || 1) >= getPaginatedResults(assignment.id).totalPages}
                          ...
                          <div>
                            <p className="text-sm text-gray-700">
                              ...
                              <span className="font-medium">
                                {((currentPage[assignment.id] || 1) - 1) * ITEMS_PER_PAGE + 1}
                              </span>
                              {' '}to{' '}
                              <span className="font-medium">
                                {Math.min(
                                  (currentPage[assignment.id] || 1) * ITEMS_PER_PAGE,
                                  getPaginatedResults(assignment.id).totalItems
                                )}
                              </span>
                              {' '}of{' '}
                              <span className="font-medium">{getPaginatedResults(assignment.id).totalItems}</span>
                              {' '}students
                            </p>
                          </div>
                          <div>
                            <nav className="relative z-0 inline-flex rounded-md shadow-sm -space-x-px" aria-label="Pagination">
                              ...
                              {[...Array(getPaginatedResults(assignment.id).totalPages)].map((_, i) => (
                                <button
                                  key={i}
                                  onClick={() => handlePageChange(assignment.id, i + 1)}
                                  className={`relative inline-flex items-center px-4 py-2 border text-sm font-medium ${
                                    (currentPage[assignment.id] || 1) === i + 1
